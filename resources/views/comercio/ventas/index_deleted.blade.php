...
@section('content')
  ...
  <i class="fas fa-tags fa-stack-1x fa-lg"></i>
   <i class="fas fa-ban fa-stack-1x fa-2x text-danger"></i>
                    ...
                    <a href="{{ route('ventas.index') }}">Lista de ventas</a>
                        <span>/</span>
                        <span> @if ($ventas->count() === 1)
                Una venta eliminada
            @elseif ($ventas->count() > 1)
                {{ $ventas->count() }} ventas eliminadas
            @else
  ...
  @foreach($ventas as $key => $venta)
    <tr class="hoverable">
      ...
      <a onclick="restaurar_venta({{ $venta->id }},'{{ $venta->nombre }}')" class="text-success m-1" 
                    data-toggle="tooltip" data-placement="bottom" title='Restaurar la venta #{{ $venta->id }}'>
                      <i class="fas fa-2x fa-trash-restore"></i>
                            </a>
                
                            <a onclick="eliminar_venta({{ $venta->id }},'{{ $venta->nombre }}')" class="text-danger m-1" 
                    data-toggle="tooltip" data-placement="bottom" title='Eliminar definitivamente la venta #{{ $venta->id }}'>
                      <i class="fas fa-2x fa-trash"></i>
                            </a>
                            <form id="restaurar{{ $venta->id }}" method="POST" action="{{ route('ventas.deleted.update', $venta->id) }}" accept-charset="UTF-8">
    <input name="_method" type="hidden" value="PUT">
    {{ csrf_field() }}
</form>
                            <form id="eliminar{{ $venta->id }}" method="POST" action="{{ route('ventas.deleted.destroy', $venta->id) }}" accept-charset="UTF-8">
...
function eliminar_venta(id,nombre){
    swal({
  title: 'Eliminar venta',
  text: '¿Desea eliminar definitivamente la venta "'+nombre+'"?',
  type: 'warning',
...
function restaurar_venta(id,nombre){
    swal({
  title: 'Restaurar venta',
  text: '¿Desea restaurar la venta "'+nombre+'"?',
  type: 'question',
...
var datetime =  moment().format('DD MMMM YYYY, h-mm-ss a'); 
    var titulo_archivo = "Lista de ventas eliminados ("+datetime+")";
     $('#dtventas').DataTable( {
        dom: 'Bfrtip',
    lengthMenu: [
        ...
        responsive: {
            details: {
                display: $.fn.dataTable.Responsive.display.modal( {
                    header: function ( row ) {
                        var data = row.data();
                        return '<span class="fa-stack"><i class="fas fa-tags fa-stack-1x fa-lg"></i> <i class="fas fa-ban fa-stack-1x fa-2x text-danger"></i></span> Datos de la venta eliminada "'+ data[1]+'"';
                    }
                } ),
