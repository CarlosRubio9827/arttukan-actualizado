...
@section('content')
  ...
  @foreach($compras as $key => $compra)
    <tr class="hoverable">
      ...
      <a onclick="restaurar_compra({{ $compra->id }},'{{ $compra->nombre }}')" class="text-success m-1" 
                    data-toggle="tooltip" data-placement="bottom" title='Restaurar la compra #{{ $compra->id }}'>
                      <i class="fas fa-2x fa-trash-restore"></i>
                            </a>
                
                            <a onclick="eliminar_compra({{ $compra->id }},'{{ $compra->nombre }}')" class="text-danger m-1" 
                    data-toggle="tooltip" data-placement="bottom" title='Eliminar definitivamente la compra #{{ $compra->id }}'>
                      <i class="fas fa-2x fa-trash"></i>
                            </a>
                            <form id="restaurar{{ $compra->id }}" method="POST" action="{{ route('compras.deleted.update', $compra->id) }}" accept-charset="UTF-8">
    <input name="_method" type="hidden" value="PUT">
    {{ csrf_field() }}
</form>
                            <form id="eliminar{{ $compra->id }}" method="POST" action="{{ route('compras.deleted.destroy', $compra->id) }}" accept-charset="UTF-8">
...
function eliminar_compra(id,nombre){
    swal({
  title: 'Eliminar compra',
  text: '¿Desea eliminar definitivamente la compra "'+nombre+'"?',
  type: 'warning',
...
function restaurar_compra(id,nombre){
    swal({
  title: 'Restaurar compra',
  text: '¿Desea restaurar la compra "'+nombre+'"?',
  type: 'question',
...
var datetime =  moment().format('DD MMMM YYYY, h-mm-ss a'); 
    var titulo_archivo = "Lista de compras eliminados ("+datetime+")";
     $('#dtcompras').DataTable( {
        dom: 'Bfrtip',
    lengthMenu: [
        ...
        responsive: {
            details: {
                display: $.fn.dataTable.Responsive.display.modal( {
                    header: function ( row ) {
                        var data = row.data();
                        return '<span class="fa-stack"><i class="fas fa-tags fa-stack-1x fa-lg"></i> <i class="fas fa-ban fa-stack-1x fa-2x text-danger"></i></span> Datos de la compra eliminada "'+ data[1]+'"';
                    }
                } ),
